...
| 需求 | 理由 |
|---|---|
| 1 kHz 硬实时增稳环 | FreeRTOS 双核 + 任务绑核，能保证 1 ms 周期 |
| BLE HID 主机 | `esp_hid_host` 是官方组件，不用外挂模块 |
| CAN | 片内 TWAI 控制器（**只差一颗收发器**） |
| 视觉链路 | 对外 UART1 = GPIO17/18 |
...
esp32_turret/
├── CMakeLists.txt
├── sdkconfig.defaults            # TWAI / BLE HID host / SPI / PSRAM 关键配置
├── main/
│   ├── app_main.c                # 初始化 + 创建任务
...
1. **绑核**：ESP32-S3 是双核。把 `imu_task` 和 `stabilizer_task` 绑到 **APP_CPU（核心 1）**，
   把 BLE / UART / 协议解析放到 **PRO_CPU（核心 0）**——无线协议栈的长临界区会阻塞稳定环。
...
| **IMU 布线跨转动关节** | IMU 装在托盘上，SPI 线要跨关节 | 一期用"限位自转 ±90°"规避；必须有掉线检测，异常自动旁路稳定环 |
