...
[`demos/转台演示.mp4`](demos/转台演示.mp4)（9.5 s，1.7 MB，竖屏 720×1280）
—— 2026-09-22 实拍：握手柄推右摇杆，转台做俯仰 / 自转；
屏幕里是装在托盘上的摄像头的回传画面。

> 原片 37 MB / 15.8 s。按逐帧差分的**运动强度分布**裁掉前 5.5 s
> （那段在调手柄，转台基本不动），再 x264 CRF 28 重压到 1.7 MB。
> 画质抽帧核对过：屏幕按钮字样、排线颜色、齿轮结构都清晰可辨。
> 原片留本地，不进仓库。
