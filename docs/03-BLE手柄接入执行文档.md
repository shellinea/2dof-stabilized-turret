...
**里程碑 M2：打通「CodexPad-S10 手柄 → ESP32-S3 → CAN → 两台电机」的整条链路**

> **这是一份"照着做"的手册，不是设计文档。**
> 每一步都给出：**要做什么 → 敲什么命令 → 看到什么算成功 → 没成功怎么办**。
> 协议细节（notify 载荷格式、GATT 全表、键位表）不用现在读，全在**附录 A**，写到对应代码时再翻。
>
> 第 1~6 步只用手柄 + 板子 + 两根 USB 线；**第 7 步起要 CAN 收发器和两台电机**。
> IMU 和泰山派属于后面的里程碑，本篇不涉及。

| 项 | 内容 |
|---|---|
| 版本 | v2.6（2026-09-21，M2e 收官：手柄真值驱动转台上板跑通，见 §8） |
| 关联 | `docs/00-总体方案-PRD.md` §5.4、§4.1.2、§4.1.3、里程碑 M2 |
| 关联 | [`docs/04-ESP32-CAN电机驱动.md`](04-ESP32-CAN电机驱动.md) —— M2d/M2e 的电机侧细节 |
...
| **7** | **M2d** 接 CAN（需先买收发器） | — | ✅ 2026-09-20（CAN 全通 + 速度逆解 6/0，见 §7） |
| **8** | **M2e** ω_ref 换成手柄真值，驱动转台 | 1 天 | ✅ 2026-09-21（上板确认可控转动，见 §8） |
...
第2步 PC 验证手柄 ──────────────────┘   ← 并行做，用来二分"是板子的问题还是手柄的问题"
                                    ...
                                    第5步 M2b 连接+解析  ──►  第6步 M2c 映射 ω_ref
                                                                       │
                                    第7步 M2d 接 CAN ✅ ──►  第8步 M2e ω_ref 驱动转台 ✅
                                    （相位 1~7 全通）        （上板确认可控转动）
```
...
- ❌ `D:\Users\27640\Desktop\步进电机项目\`（有中文）
- ❌ `C:\Users\27640\Desktop\新建文件夹\`
...
## 6.5 全流程总览（回看 M2a–M2e）

> 前面按里程碑分开写，代码也按职责拆在 6 个文件里 —— 每处都短，
...
```
【启动】app_main()
   nvs_flash_init → motor_bringup() ← TWAI 建节点 + 两台探活 + 读姿态 + 使能
   → nimble_port_init
   → pad_input_init()               ← 建覆盖队列 + 起 pad_m2c 任务（此时 TWAI 必须已就绪）
...
【数据流】手柄推 notify → pad_gap_event(NOTIFY_RX)
   ...
   pad_m2c 任务（另一个 task，50 Hz）xQueuePeek → 归一化+死区 → ω_ref → 变化才打印
        └─ ω_tilt/ω_pan 转成毫度/秒 → motor_diff_drive()  ──★ M2e 新增这一段
             首次 / 数值变了 / 每 200 ms 心跳 才发；发失败不更新，下一拍自动重试

【掉线】pad_gap_event(DISCONNECT) → on_disconnect()
   peer_delete + pad_gatt_reset + pad_input_center() + pad_link_scan()  ──★ 回到扫描，闭环
        └─ pad_input_center() 把状态压回中位 ⇒ ω_ref=0 ⇒ 控制环下一拍算出"值变了" ⇒ 发 0 速度
```
...
| task | 谁创建 | 干什么 | 绑核 |
|---|---|---|---|
| `nimble_host` | `app_main.c:133` → IDF 的 `nimble_port_freertos.c:43` | `nimble_port_run()` 协议栈主循环，**以及全部 GAP/GATT 回调** | **核 0（PRO_CPU）**，`CONFIG_BT_NIMBLE_PINNED_TO_CORE=0`，栈 4096，优先级 21 |
| `pad_m2c` | `pad_input.c:214` | 50 Hz 控制环：读队列 → 算 `ω_ref` → 差速逆解 → 发 CAN | 未绑（`xTaskCreate`）；将来与增稳环一起进 **核 1（APP_CPU）** |

> ⚠ `app_main.c:133` 传进去的 `pad_host_task` **只是个回调函数指针**，不是 task 本体。
> task 是 IDF 的 `esp_nimble_enable()` 建的，名字叫 `nimble_host`。
> 所以 `pad_host_task` 第一行那句 `"BLE Host 任务已启动"` 是在**新建的那个 task 里**打印的。
>
> ⚠ 从 M2e 起 `pad_m2c` 还**多了一个身份**：它是全工程唯一允许调 `motor_twai.c`
> 的任务（TWAI 发送是阻塞式的，`nimble_host` 那 4 KB 栈碰不得）。
> 详见 [`docs/04-ESP32-CAN电机驱动.md`](04-ESP32-CAN电机驱动.md) §6。
...
# 7. M2d —— 已收官（2026-09-20），运动学那半条已换成 ESP32 实测

**里程碑终点原本写的是**："串口能稳定打印解析好的摇杆/按键，`ω_ref` 数值合理。"
**这条达到了**；再往下一棒"接电机"，实测结果是**比预想的顺利** ——
CAN 侧 7 个相位的自检固件（片内回环 → 跨引脚 → 真总线 → 位置模式 → 差速 → 速度模式）
已全过，其**已验证的那部分**已经抽成 `firmware/esp32_turret/main/motor_twai.c`，
细节另见 [`docs/04-ESP32-CAN电机驱动.md`](04-ESP32-CAN电机驱动.md)。

**⚠ 2026-09-20 补记：本节原来写的"运动学是现成的、那是搬运不是探索"低估了工作
量，也漏了唯一一处需要新判断的地方。** 逐条说：

| 原话 | 实际情况 |
|---|---|
| 「差速逆运动学 `tools/gimbal.py` 已验证」 | 对，但 **C 侧要从零写一遍**（`pulses_of_mdeg`/`cmd_angle`/`world_of_pos`/`move_sync`/`read_pose_mdeg`/`diff_drive`），并重新在电机上验一次。位置模式与速度模式**各验一遍**，两条独立证据互相印证 |
| 「CAN 帧 `tools/zdt_can.py` 已验证」 | 对，**C 侧同样要重写**：多帧拆分、地址在 ID 里（不在数据里）、扩展帧、`FF 66` 广播触发 |
| 收发器 | 买了，和零散线一起到的 |
...
| 5 | 参数写诊断 | 6 条形状变体全回 `00 EE`（见下） |
| 6 | 差速 + 2 号镜像补偿 | 5/0：纯俯仰 Δ俯仰+10.34°/Δ自转+0.30°，纯自转反过来 |
| 7 | **速度模式差速逆解** | 6/0：纯俯仰实测俯仰 **+36.62°/s**/自转 +0.33°/s，纯自转俯仰 −0.61°/s/自转 **+37.29°/s** |

**速度逆解的写法（命令空间，2 号 mirror=true）**：

```
cmd1 = ω_tilt + ω_pan
cmd2 = ω_pan − ω_tilt        /* = −(ω_tilt − ω_pan)，2 号镜像取反 */
```

纯俯仰 ⇒ cmd1=+V, cmd2=−V；纯自转 ⇒ cmd1=+V, cmd2=+V。这和相位 6 位置模式下
实测的符号组**逐条对上**。

**⚠ 三个只有接了电机才会发现的坑**（PC 端永远碰不到，重跑前先看）：

1. **CAN 节点不收自己发出的帧**（协议规定，不是 bug）—— 单节点验物理通路**不能
   拿「收到帧」当判据**。正确判据是 `tx_err / rx_err / 累计总线错误 = 0`：
   发送时硬件必须逐位监听总线，位没回来就报位错误。
2. **`enable_self_test` 和 `enable_loopback` 不是一回事**：前者=「发送不需要 ACK」，
   总线上有真电机时**必须关**，否则"自己给自己盖章"，电机没接也能过。
3. **帧会丢，而且不声不响**。相位 7 是"发完就不管"，实测遇到过一轮 2 号电机的
   `FD` 没收到（Δ=+0.02°，指令 −10°），那一轮总线上同时有 `填充错误` / `仲裁丢失`。
   重跑就好了。⇒ 后面接增稳环（100 Hz 连续发速度指令）时**必须加应答核对或看门狗**，
   否则"发出去电机没收到"会直接变成控制失效。

**改位置到达窗口 `D1 07 <store> <窗口2B>` 在本机固件 V2.0.4 上是死命令**（已穷尽）：
6 条形状变体全回 `00 EE`，含"把当前值 0.8 原样写回"和"去掉辅助码"两种 ⇒ 排除取值
不在允许集、排除辅助码错。同族 `D3 56`（改过热过流）回 `02 执行正确`，说明写机制
本身是好的。唯一办法是电机自己的 OLED 菜单改 `PRWindow`。**影响面只限位置模式的
收尾残差**（到位判据 = `|目标−实际| < 窗口`，默认 0.8° ⇒ 冲过 ~0.5° 就宣布到位），
增稳主线走速度模式，压根没有到位窗口 ⇒ **不阻塞主线**。

---

# 8. M2e —— 已收官（2026-09-21）：手柄真值驱动转台

链条**最后一节已接上并上板跑通**：

```
摇杆 ──► ω_ref ──► 差速逆运动学 ✅已验证 ──► CAN ──► 电机 ✅已验证
        （M2b/M2c）  （相位 6/7）      （相位 1~3）
```

**实测结果**：用户上板操作确认「能控制的转动了」—— 手柄摇杆驱动转台正常。
串口日志同时证实 CAN 链路正常：`[motor] TWAI 就绪：tx=GPIO4 rx=GPIO5 500 kbps`、
`1 号 +28.690 / 2 号 -10.321`、初始姿态 `俯仰=+19.505 度 自转=+9.184 度`。

**实现落点**（不在 `main.c` 里从头写，而是抽成了独立模块）：

| 文件 | 内容 |
|---|---|
| `firmware/esp32_turret/main/motor_twai.h` | 对外 API + **线程契约**（只有 `pad_input.c` 的 `m2c_task` 能调，因为收发函数会阻塞，而 NimBLE host task 栈只有 4 KB） |
| `firmware/esp32_turret/main/motor_twai.c` | TWAI 建节点、Emm 拆帧、`motor_read_pose()`、`motor_diff_drive(wt, wp, acc)` |
| `firmware/esp32_turret/main/pad_input.c` | `m2c_task` 里把 `ω_tilt/ω_pan`（float °/s）转成毫度/秒（int32）后调 `motor_diff_drive()` |
| `firmware/esp32_turret/main/app_main.c` | `motor_bringup()`：上电探活两台电机 + 读一次姿态 + 使能 |

**接入时的三条判断**：

1. **不是每拍都发**。速度指令是**有状态的**（电机保持上次收到的速度），
   所以策略是「第一次 / 数值变了 / 每 200 ms 心跳」。
   只按变化发省总线，但会丢的如果是停止指令，转台就停不下来 ⇒ 200 ms 心跳兜底。
   发失败**不更新**已发送值，下一拍（20 ms 后）自动重试。
2. **单位全程用定点**（毫度/秒）。`%f` 在新库下可能不打印，而且 twai 路径上
   不想引入浮点除法；`rpm = mdps/6000` 四舍五入在 `rpm_of_mdps()` 里一次做完。
3. **停转走 `pad_input_center()`**，不走直接 `motor_stop()`。GAP 断开回调里把
   手柄状态清成回中 ⇒ `ω_ref` 归零 ⇒ 控制环下一拍自然发出 0 速度。

**仍未钉死的一项（不阻塞）**：`OMEGA_MAX_DPS` 取 60 °/s 是估计值，
以及左右杆的符号与机构正方向的对齐（相位 7 已给出符号基准 `cmd1=ω_tilt+ω_pan`，
真要反了只需把某个轴取负，不用改结构）。上板觉得手感不对再调。
